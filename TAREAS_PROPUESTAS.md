...
**Título:** Agregar prueba E2E de humo para contenidos críticos (releases y galería).

**Problema:** No hay pruebas que validen presencia de secciones clave y carga de assets.

**Propuesta de implementación:**
- Incorporar Playwright con una prueba de smoke que verifique:
  - Renderizado de tarjetas de releases y videos.
  - Ausencia de imágenes rotas (status 404) en recursos locales críticos.
  - Existencia y funcionamiento básico de filtros (una vez implementados).
- Añadir script npm de ejecución en CI (por ejemplo `npm run test:e2e`).

**Criterios de aceptación:**
- La prueba falla si una portada de release no carga.
- La prueba falla si no aparece la UI de filtros tras su implementación.
- La prueba se ejecuta localmente con un solo comando documentado.
